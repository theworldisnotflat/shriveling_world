...
namespace shriveling {
    ...
    export class ConeShaderBackWorker {
        private _self: any;
...
let worker: shriveling.ConeShaderBackWorker;
self.addEventListener('message', (event) => {
    'use strict';
    if (worker === undefined) {
        worker = new shriveling.ConeShaderBackWorker(<any>self);
    }
    let datas = event.data.data;
    let action = event.data.action;
    if (action === 'init') {
        let towns = <{ [cityCode: string]: shriveling.NEDLocal }>JSON.parse(datas.towns, shriveling.reviver);
        let bboxes = <shriveling.IBBox[]>JSON.parse(datas.bboxes, shriveling.reviver);
        worker.initData(
            towns,
            bboxes,
            datas.cones,
