...
export default class BigBoard {
    ...
    constructor() {
        prepareConfiguration().then(() => {
            ...
            let gui=new GUI(this, container, this._merger);
            // tslint:disable-next-line
            new dataSetManager(this,gui);
            this._animate();
        });
    }
    /**
    ...
    public rescaleText(): void {
        for (var i = this._geometryText.children.length - 1; i >= 0; i--) {
            this._geometryText.children[i].scale.set(
                this._sizetext,
                this._sizetext,
                this._sizetext
            );
    ...
    public updateCityName(): void {
        ...
        for (var j = 0; j < this.getMergerI.Cities.length / 2; j++) {
            ...
            if (population > this._populations) {
                ...
                let x =
                    -CONFIGURATION.THREE_EARTH_RADIUS *
                    1.1 *
                    Math.cos(cart.latitude * 0.95) *
                    Math.cos(cart.longitude);
                let y =
                    CONFIGURATION.THREE_EARTH_RADIUS *
                    1.1 *
                    Math.sin(cart.latitude * 0.95);
                let z =
                    CONFIGURATION.THREE_EARTH_RADIUS *
                    1.1 *
                    Math.cos(cart.latitude * 0.95) *
                    Math.sin(cart.longitude);
    ...
    private _init(): HTMLDivElement {
        ...
        this._cameraP = new PerspectiveCamera(
            45,
    ...
    private exporterOBJ(): void {
        ...
        this.coneBoard.lineCollection.forEach(line => {
            if (
                line.getTheta <
                2000 / (CONFIGURATION.earthRadiusMeters / 1000)
            ) {
        ...
        let blobCone = new Blob([exporter.parse(groupCone)], {
            type: 'text/plain;charset=utf-8',
        });
        save(blobCone, 'sceneCones.obj');
        let blobLineShort = new Blob([exporter.parse(groupLineShortHaul)], {
            type: 'text/plain;charset=utf-8',
        });
        save(blobLineShort, 'sceneLinesShortHaul.obj');
        let blobLineLong = new Blob([exporter.parse(groupLineLongHaul)], {
            type: 'text/plain;charset=utf-8',
        });
    ...
    private _animate(): void {
        ...
        let camera =
            this.orthographique === true ? this._cameraO : this._cameraP;
