...
export class GUI {
    ...
    public constructor(
        ...
        conf = {
            ...
            'cones color':
                '#' +
                (<any>CONFIGURATION.BASIC_CONE_MATERIAL).color
                    .getHex()
                    .toString(16),
            'cones transparency': CONFIGURATION.BASIC_CONE_MATERIAL.opacity,
            'line color':
                '#' +
                CONFIGURATION.BASIC_LINE_MATERIAL.color.getHex().toString(16),
            'text color':
                '#' +
                CONFIGURATION.BASIC_TEXT_MATERIAL.color.getHex().toString(16),
            ...
            'standard parrallel 1':
                CONFIGURATION.standardParallel1 * CONFIGURATION.rad2deg,
            'standard parrallel 2':
                CONFIGURATION.standardParallel2 * CONFIGURATION.rad2deg,
            'with limits': true,
    ...
    public async filesToInsert(list: IListFile[]): Promise<number> {
        ...
        Promise.all([
                    ...
                    bigBoard.countryBoard.add(JSON.parse(json)).then(() => {
                        ...
                        bigBoard.countryBoard.countryMeshCollection
                            ...
                            .forEach(country => {
                                ...
                                let folder = countryFolder.addFolder(
                                    countryName
                                );
                                folder
                ...
                if (
                    ...
                    this._merger.transportNames.lines.forEach(transportName => {
                        ...
                        function lineListener(): void {
                            ...
                            let color = parseInt(
                                lineColor.getValue().replace('#', ''),
                                16
                            );
                            bigBoard.coneBoard.lineCollection
                                .filter(
                                    line => transportName === line.transportName
                                )
                                .forEach(line => {
                        ...
                        let lineColor = folder
                            .addColor(conf, 'line color')
                            .name('color');
                    ...
                    this._merger.transportNames.cones.forEach(transportName => {
                        let folder = terresterialFolder.addFolder(
                            transportName
                        );
            ...
            if (
                bigBoard.countryBoard.ready === true &&
                bigBoard.state === 'complete'
            ) {
                flagTransportDone = false;
                annees
    ...
    private _initInteraction(container: HTMLElement): void {
        ...
        conf['ambient color'] =
            '#' + bigBoard.ambient.color.getHex().toString(16);
        conf['exportCountry'] = bigBoard.orthographique;

        // light
        let lightFolder = gui.addFolder('Light');
        lightFolder
            .add(bigBoard.ambient, 'intensity', 0, 5, 0.01)
        ...
        lightFolder
            .add(bigBoard.light.position, 'x', -100, 100, 1)
            .onChange(() => bigBoard.helper.update());
        lightFolder
            .add(bigBoard.light.position, 'y', -100, 100, 1)
            .onChange(() => bigBoard.helper.update());
        lightFolder
            .add(bigBoard.light.position, 'z', -100, 100, 1)
            .onChange(() => bigBoard.helper.update());
        lightFolder
            .add(bigBoard.light.shadow.mapSize, 'width', 0, 1000)
            .step(1);
        lightFolder
            .add(bigBoard.light.shadow.mapSize, 'height', 0, 1000)
            .step(1);
        lightFolder
            .add(bigBoard.light.shadow.camera, 'near', 0, 1000)
            .step(0.5);
        ...
        let refLong = referenceFolder
            .add(conf, 'longitude', -180, 180)
            .step(0.01);
        refLong.onChange(changeReference);

        let refLat = referenceFolder
            .add(conf, 'latitude', -89.99, 89.99)
            .step(0.01);
        refLat.onChange(changeReference);
        let refHeight = referenceFolder
            .add(conf, 'hauteur', -radius + 10, radius + 10)
            .step(1000);
        ...
        referenceFolder
            ...
            .onChange(
                (v: number) =>
                    (CONFIGURATION.standardParallel1 =
                        v * CONFIGURATION.deg2rad)
            );
        referenceFolder
            .add(conf, 'standard parrallel 2', -90, 90, 0.1)
            .onChange(
                (v: number) =>
                    (CONFIGURATION.standardParallel2 =
                        v * CONFIGURATION.deg2rad)
            );
        projectionFolder
            .add(CONFIGURATION, 'projectionInit', conf.projection)
            .name('initial projection');
        projectionFolder
            .add(CONFIGURATION, 'projectionEnd', conf.projection)
        ...
        coneFolder
            .add(bigBoard, 'withLimits')
            .onChange((value: boolean) => (conf['with limits'] = value));
        ...
        let that = this;
        DragnDrop(container, that.filesToInsert, this);
    }
}
