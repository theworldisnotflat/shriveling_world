...
import { Vector2, Face3, Vector3, Geometry } from 'three';
import { CONFIGURATION } from '../common/configuration';
import { Cartographic, mapProjectors } from '../common/utils';
import { IBBox, ITypeExtrusion } from '../definitions/project';
...
interface IPreGeometry {
    ...
    faces: Face3[];
    morphs: { [name: string]: Vector3[] };
    verticesExtruded: Vector3[];
    morphsExtruded: { [name: string]: Vector3[] };
...
function prepareGeometry(verticesAndTriangles: IVerticesTriangles, mainProjector: string): IPreGeometry {
    ...
    let vertices: { [name: string]: Vector3[] } = {};
    let verticesExtruded: { [name: string]: Vector3[] } = {};
    let uvs: Vector2[] = [];
    let faces: Face3[] = [];
    let faceVertexUvs: Vector2[][] = [];
    ...
    cartoVertices.forEach((carto) => {
        let lookupPos = <{ [x: string]: Vector3 }>carto.toTHREEVector3();
    ...
    let ia: number, ib: number, va1: Vector3,
        vb1: Vector3, va2: Vector3, vb2: Vector3, normal: Vector3;
    ...
    for (let i = 0; i < mainPolygon.length; i++) {
        ...
        faces.push(new Face3(ia, ib, n + ia, [normal, normal, normal]));
        faceVertexUvs.push([uvs[ia], uvs[ib], uvs[ia]]);
        faces.push(new Face3(ib, n + ib, n + ia, [normal, normal, normal]));
    ...
    holePolygons.forEach((holeIndexes) => {
        for (let i = 0; i < holeIndexes.length; i++) {
            ...
            faces.push(new Face3(ib, ia, n + ia, [normal, normal, normal]));
            faceVertexUvs.push([uvs[ib], uvs[ia], uvs[ia]]);
            faces.push(new Face3(ib, n + ia, n + ib, [normal, normal, normal]));
...
export class CountryGeometry extends Geometry {
    ...
    private constructor(
        ...
        this.morphTargets = [];
        let list: Vector3[], reverse: IReverseLookupExtrusion, morphName: string;
