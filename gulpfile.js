...
const doc = shell.task(
  'typedoc --out documentation/html --json documentation/json.json --name "shriveling the world" --ignoreCompilerErrors --hideGenerator --target ES6  src'
);
...
const zipper = async done => {
  await Promise.all(
    getDirectories("datasets").map((directory, i) => {
      Zip.archiveFolder(
        sources.datasets + directory,
        destinations.datasets + directory + ".zip"
      );
    })
  );
  done();
